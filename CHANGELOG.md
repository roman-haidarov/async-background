...
- **P0 — graceful shutdown drain.** `Runner#run` used `semaphore.acquire {}` as
  a poor-man's barrier after `scheduler_loop` exited. That call only blocks
  while the semaphore is **saturated**; with any free slot it returns
  immediately. Result: on `SIGTERM`/`SIGINT`, the runner would close the queue
  `Store` and remove its UNIX socket while a job was **still executing**.
  An overlapping restart of the same `worker_index` would then `recover` the
  half-done row and run it a second time on the new process.

  Fix: `Async::Semaphore.new(job_count, parent: @drain_barrier)` makes every
  spawned job task a child of a dedicated `Async::Barrier`. Shutdown now waits
  on `@drain_barrier.wait` before closing the store and socket, so a job is
  guaranteed to terminate inside the process that started it.

- **Lease tokens (`claim_token`) on every terminal transition.** `complete`,
  `fail`, and `retry_or_fail` previously updated rows by `id` only.
  In an overlap-restart scenario, an old worker could legally close a row
  that had already been re-claimed by a new worker, double-counting metrics
  and stealing attribution. All terminal SQL is now CAS:
  `WHERE id = ? AND claim_token = ? AND status = 'running'`. Stale leases
  return `false` / `nil` and the runner logs a warning instead of bumping
  metrics or writing a misleading "completed" log line.

  Note: lease tokens guarantee honest DB state, not idempotent side effects.
  Jobs with external effects must still be idempotent by `job.id` — this is
  the at-least-once execution contract for the queue.

- **Metrics and logs are written only after a successful CAS.** Previously
  `metrics.job_finished` and the "completed" log line ran before the SQL
  `UPDATE`, so even a row that the worker no longer owned would bump
  `total_successes` / `total_failures` / `total_timeouts`. Now every metric
  increment and every terminal log line is gated on the CAS result.

### Metrics compatibility and dashboard wiring

- **Keep `async-utilization` optional while supporting 0.3 and 0.4.** Metrics now use
  cached `Registry#metric` handles, the API available in both supported versions. This fixes
  `NoMethodError` on 0.4, which removed `Registry#increment`, `#decrement`, and `#set`.
  The gem remains a development dependency only; applications opt in explicitly.

- **Dashboard-safe metrics reads.** `Metrics.available?` exposes whether the optional
  integration can load. `Metrics.read_all` returns `[]` before workers create the file or
  when the gem is absent, validates the worker count, and takes a shared file lock while
  mapping the snapshot. `ASYNC_BACKGROUND_METRICS_PATH` and `Runner#metrics_shm_path` make
  the shared-memory location explicit for multi-container deployments.

### Features for upcoming Web dashboard

- **Lifecycle columns on `jobs`.** Added `claim_token`, `started_at`,
  `finished_at`, `duration_ms`, `last_error_class`, `last_error_message`.
  `fetch()` returns the freshly generated `claim_token` so the runner can
  thread it through every subsequent call. `mark_started!(id, claim_token:)`
  stamps `started_at` immediately before `perform`, letting a dashboard
  distinguish *claimed-but-waiting* (semaphore queue) from *executing*
  without needing a separate enum value.

- **`Store#next_pending_run_at`** — single `MIN(run_at)` query backed by the
  existing partial index `idx_jobs_pending`. Used by the queue listener to
  wake exactly when the next delayed/retry job is due.

- **`Store#data_version`** — wraps `PRAGMA data_version`. The dashboard
  watches this on a dedicated read-only connection and skips snapshot work
  when nothing in the DB has changed since the last poll, eliminating the
  need for an event log on the queue's write hot path.

- **New composite index `idx_jobs_status_finished_at`** for the dashboard's
  "recent done / recent failed" queries.

### Other fixes

- **Delayed and retry precision (`push_in` / `retry_delay`).** The queue
  listener slept up to `QUEUE_POLL_INTERVAL` (5s) regardless of when the next
  job was due, so `push_in(2)` and `retry_delay: 1` both rounded up to ~5s.
  The listener now waits `min(next_pending_run_at - now, QUEUE_POLL_INTERVAL)`
  with a `MIN_QUEUE_WAIT = 0.001` floor for overdue rows.

- **Cleanup retention reworked.** Previously `DELETE FROM jobs WHERE status =
  'done' AND created_at < ?` — a job created hours ago and finished a second
  ago could be purged at the next cleanup tick, making it invisible to a
  dashboard. Cleanup now uses `finished_at`. Failed jobs get their own
  retention window via `FAILED_RETENTION_AGE` (default 7 days) so they remain
  available for inspection / retry without bloating the table indefinitely.
  Pre-existing terminal rows are backfilled with `finished_at = created_at`
  during the migration.

- **`recover(worker_id)` now also clears `claim_token` and `started_at`** on
  the requeued rows so the next fetch issues a fresh lease.

- **`last_error_message` is truncated to `ERROR_MESSAGE_MAX_LEN` (2000 chars)**
  to keep multi-megabyte tracebacks out of the row.

### Breaking changes

- `Store#complete(id)` → `Store#complete(id, claim_token:, finished_at: nil, duration_ms: nil)`.
  Returns `true` on a successful transition, `false` on stale lease.
- `Store#fail(id)` → `Store#fail(id, claim_token:, error_class: nil, error_message: nil, finished_at: nil, duration_ms: nil)`.
  Returns `true` / `false`.
- `Store#retry_or_fail(id, fallback_options:)` → `Store#retry_or_fail(id, claim_token:, error_class: nil, error_message: nil, fallback_options: nil, finished_at: nil, duration_ms: nil)`.
  Returns `:retried` / `:failed` / `nil` (nil on stale lease).
- `Store#fetch` result hash now includes the `claim_token` key.

  Users who call `Store` directly (e.g. for custom enqueue/dispatch) must
  propagate the `claim_token` returned by `fetch`. The `Runner` already does
  this for all built-in paths.
