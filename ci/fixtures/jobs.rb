require 'async/background/job'
require 'json'

module CIJobs
  LEDGER_PATH = ENV.fetch('LEDGER_PATH', 'tmp/ci_ledger.log')

  def self.setup_ledger!
    FileUtils.mkdir_p(File.dirname(LEDGER_PATH)) if defined?(FileUtils)
    File.write(LEDGER_PATH, '')
  end

  def self.record_execution!(job_class, job_arg)
    line = JSON.generate(
      job_class:   job_class,
      job_arg:     job_arg.to_s,
      worker_pid:  Process.pid,
      executed_at: Process.clock_gettime(Process::CLOCK_REALTIME)
    ) + "\n"

    File.open(LEDGER_PATH, File::WRONLY | File::APPEND | File::CREAT, 0644) do |f|
      f.flock(File::LOCK_EX)
      f.write(line)
      f.flush
    end
  end

  def self.read_ledger
    return [] unless File.exist?(LEDGER_PATH)
    File.foreach(LEDGER_PATH).map { |l| JSON.parse(l) }
  end

  class FastJob
    include Async::Background::Job
    def perform(n)
      CIJobs.record_execution!(self.class.name, n)
    end
  end

  class SlowJob
    include Async::Background::Job
    def perform(n)
      sleep(0.5 + rand * 0.5)
      CIJobs.record_execution!(self.class.name, n)
    end
  end

  class FailingJob
    include Async::Background::Job
    def perform(n)
      CIJobs.record_execution!(self.class.name, n)
      raise "CIJobs::FailingJob intentional error for job #{n}"
    end
  end

  class HeartbeatJob
    include Async::Background::Job
    def perform
      CIJobs.record_execution!(self.class.name, "heartbeat")
    end
  end
end
